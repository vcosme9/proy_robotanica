...
var createHttpsProxyAgent = _interopDefault(require('https-proxy-agent'));
...
class HttpClient {
  ...
  async sendPostRequestAsync(url, options, cancellationToken) {
    const request = {
      ...
      /* istanbul ignore next */
      data: options && options.body || msalCommon.Constants.EMPTY_STRING,
...
class ClientApplication {
  ...
  async createAuthority(authorityString, azureRegionConfiguration, requestCorrelationId, azureCloudOptions) {
    ...
    return await msalCommon.AuthorityFactory.createDiscoveredInstance(authorityUrl, this.config.system.networkClient, this.storage, authorityOptions);
  }
  /**
   * Clear the cache
   */
...
class ClientAssertion {
  ...
  static parseCertificate(publicCertificate) {
    ...
    while ((matches = regexToFindCerts.exec(publicCertificate)) !== null) {
      ...
      certs.push(matches[1].replace(/\n/, msalCommon.Constants.EMPTY_STRING));
    }
...
class ConfidentialClientApplication extends ClientApplication {
  ...
  async acquireTokenOnBehalfOf(request) {
    ...
    try {
      const clientCredentialConfig = await this.buildOauthClientConfiguration(validRequest.authority, validRequest.correlationId, undefined, undefined, request.azureCloudOptions);
